...
class nosql {
  ...
  _get_table(table_name) {
    if (!(table_name in this._tables)) {
      ...
      if (env_name in process.env) {
        ...
        throw new Error(`Couldn't find an environment variable ${env_name} for table ${table_name}`);
      }
    }
  ...
  async insert(table_name, primary_key, secondary_key, data) {
    ...
    await this.client.put({ TableName: this._get_table(table_name), Item: data }).promise();
  }
  ...
  async get(table_name, primary_key, secondary_key) {
    ...
    const res = await this.client.get({ TableName: this._get_table(table_name), Key: key }).promise();
    return res.Item;
  }
  ...
  async delete(table_name, primary_key, secondary_key) {
    ...
    key[primary_key[0]] = primary_key[1];
    key[secondary_key[0]] = secondary_key[1];
    await this.client.delete({ TableName: this._get_table(table_name), Key: key }).promise();
  }
